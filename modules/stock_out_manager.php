...
if ($search) {
    $searchClause = "WHERE s.id LIKE ? OR s.invoice_no LIKE ? OR c.name LIKE ?";
    $params = ["%$search%", "%$search%", "%$search%"];
}

// Get total count
$countSql = "SELECT COUNT(*) as total FROM stock_out s LEFT JOIN companies c ON s.company_id = c.id";
if ($searchClause) {
